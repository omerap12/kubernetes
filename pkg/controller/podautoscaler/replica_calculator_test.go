...
// testDeploymentRef is the default object reference used by object-metric tests.
...
// testExternalSelector is the default label selector used by external-metric tests.
...
type cpuResource struct {
	...
	// metrics client for index i. Used to simulate stale metrics pointing at
	// pods that are no longer known.
	podNames []string
}
...
// replicaCalcSetup bundles a ReplicaCalculator wired up against fakes built
// from a calcScenario. Tests call the method they care about directly on
// setup.calc, then assert on the returned values inline.
type replicaCalcSetup struct {
	calc      *ReplicaCalculator
	ctx       context.Context
	selector  labels.Selector
	namespace string
	// tolerances used by every call. Tests can override this field before
	// calling the calculator to exercise configurable-tolerance behavior.
	tolerances Tolerances
}

// newReplicaCalcSetup builds fake pod, metrics, custom-metrics, and
// external-metrics clients that reflect the fixture, wires them into a
// ReplicaCalculator, and returns a ready-to-use setup.
...
// reflecting the fixture's per-pod state.
...
// newFakeResourceMetricsClient returns a fake metrics.k8s.io client that lists
// per-container CPU usage according to f.resource.levels.
...
// newFakeCustomMetricsClient returns a fake custom-metrics client that serves
// per-pod or single-object metric values according to f.metric.
...
// assertMetricReplicas checks the result of any usage-returning SUT method.
...
// runResourceCases calls GetResourceReplicas for each case and asserts on the returned replica count / utilization / raw value / error.
func runResourceCases(t *testing.T, cases []resourceCase) {
...
func TestReplicaCalcResourceScale(t *testing.T) {
	runResourceCases(t, []resourceCase{
		{
			name: "scale up",
			fixture: calcScenario{
				currentReplicas: 3,
...
// TestReplicaCalcResourceMissingMetrics covers GetResourceReplicas behavior when metric data is incomplete, superfluous, or otherwise partial.
...
// TestReplicaCalcResourceErrors covers error paths of GetResourceReplicas.
func TestReplicaCalcResourceErrors(t *testing.T) {
	runResourceCases(t, []resourceCase{
		{
...
// podMetricCase describes a single GetMetricReplicas test: fixture plus the
// custom-metric target value and expected outputs.
type podMetricCase struct {
	name    string
	fixture calcScenario
...
func TestReplicaCalcPodMetric(t *testing.T) {
	// Convenience builder to keep case literals short.
...
// objectMetricCase describes a single GetObjectMetricReplicas test.
type objectMetricCase struct {
	name    string
	fixture calcScenario
...
func TestReplicaCalcObjectMetric(t *testing.T) {
	...
	cases := []objectMetricCase{
		...
		},
		{
			...
			targetUsage:      10000,
			expectedReplicas: 5, // Would be 15 if we did not ignore unready pods.
			expectedUsage:    50000,
		},
		{
			name: "scale down",
...
// objectPerPodMetricCase describes a single GetObjectPerPodMetricReplicas test.
type objectPerPodMetricCase struct {
	name    string
	fixture calcScenario
...
// externalMetricCase describes a single GetExternalMetricReplicas test.
type externalMetricCase struct {
	name    string
	fixture calcScenario
...
// externalPerPodMetricCase describes a single GetExternalPerPodMetricReplicas test.
type externalPerPodMetricCase struct {
	name    string
	fixture calcScenario
...
func TestReplicaCalcExternalPerPodMetric(t *testing.T) {
	...
	cases := []externalPerPodMetricCase{
		{
...
// TestReplicaCalcComputedToleranceAlgImplementation is a regression test which
// back-calculates a minimal percentage for downscaling based on a small
// percentage increase in pod utilization which is calibrated against the
// tolerance value.
func TestReplicaCalcComputedToleranceAlgImplementation(t *testing.T) {
	startPods := int32(10)
	// 150 mCPU per pod.
	totalUsedCPUOfAllPods := int64(startPods * 150)
	// Each pod starts out asking for 2X what is really needed, so we have a
	// 50% used/requested ratio.
	...
	// Spread the request over 10 pods. Jitter is added in requests below.
	...
	buildFixture := func(replicas int32) calcScenario {
		return calcScenario{
	...
	// First pass: a minimal scale-down event fires because we've breached the
	// tolerance threshold.
	f1 := buildFixture(startPods)
	...
	// Second pass: choose a target very close to the tolerance, so no scaling
	// should happen.
	target = math.Abs(1/(requestedToUsed*(1-defaultTestingTolerance))) + .004
	finalCPUPercentTarget = int32(target * 100)

	f2 := buildFixture(startPods)
