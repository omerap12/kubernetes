...
func createClients(t testing.TB, config *restclient.Config) (testClients, *atomic.Value) {
...
func createTestNamespace(t testing.TB, c *clientset.Clientset) *corev1.Namespace {
...
func startHPAControllerAndWaitForCaches(tb testing.TB, clients testClients) {
	tb.Helper()

	ctx := tb.Context()
	discoveryClient := memory.NewMemCacheClient(clients.apiServer.Discovery())
	restMapper := restmapper.NewDeferredDiscoveryRESTMapper(discoveryClient)
...
func waitForDeploymentCondition(ctx context.Context, cs *clientset.Clientset, d *appsv1.Deployment,
	...
	timeout := 5 * hpaControllerResyncPeriod
